...
import {  registerThunk } from "@/base/store/thunks/auth.thunk";
...
export default function Register() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (data.password !== data.repass) {
            setErrorRepass('Password is not the same')
            return;
        }
        else setErrorRepass('')
        const result = await dispatchAsyn(registerThunk(data))
        if (registerThunk.fulfilled.match(result)) {
            router.push("/login");
        }
    }
    const validRePass = (value: string) => {
        setErrorRepass('')
